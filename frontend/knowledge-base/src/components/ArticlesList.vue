            ...
            <v-row>
                <v-col class="column_wrapper">
                    <li v-for="(articles, index) in articlesSortedByFirstLetter" v-bind:key="index"
                        ...
                        <v-row class="font-weight-bold font-weight-black blue--text"><h2>{{ index.toUpperCase()}}</h2>
                        </v-row>
                        <v-row v-for="(article) in articles" v-bind:key="article.id">
                            <a :href="article.URL" target="_blank">{{ article.Title }}</a>
                            <v-icon small class="padding" color="black"
